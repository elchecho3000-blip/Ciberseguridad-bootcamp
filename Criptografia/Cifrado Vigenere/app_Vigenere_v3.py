...
def mostrar_estadisticas(historial, contador):
    ...
    if not historial:
        ...
        cifrados = sum(1 for h in historial if h["operacion"] == "Cifrado")
        descifrados = sum(1 for h in historial if h["operacion"] == "Descifrado")
        print(f"Mensajes cifrados   : {cifrados}")
        print(f"Mensajes descifrados: {descifrados}")
        print(f"Total operaciones   : {contador}")
